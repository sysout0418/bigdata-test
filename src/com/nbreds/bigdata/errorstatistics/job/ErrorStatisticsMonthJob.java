...
public class ErrorStatisticsMonthJob implements Job {

	protected static final Logger logger = Logger.getLogger(ErrorStatisticsMonthJob.class);

	public static void main(String[] args) {
		System.setProperty("SERVER_TYPE", "dev");
		ErrorStatisticsMonthJob job = new ErrorStatisticsMonthJob();
		try {
			job.execute(null);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		if (CommonConstants.errorStatisticsMonthIsRunning) {
			logger.info("Process Still Running....");
			CommonConstants.errorStatisticsMonthRunCheckCount++;
			/**
			 * 싱글톤 방식이나 설정된 최대 스킵 카운트 이상으로 동작하지 못했을경우 프로세스이상으로 간주하고 새로이 시작
			 */
			if (CommonConstants.errorStatisticsMonthRunCheckCount > CommonConstants.maxRunCheckCount) {
				CommonConstants.errorStatisticsMonthRunCheckCount = 0;
			} else {
				return;
			}
		} else {
			CommonConstants.errorStatisticsMonthIsRunning = true;
		}

		try {
			ErrorStatisticsDao dao = ErrorStatisticsDao.getInstance();
			ErrorStatisticsVo errorStatVo = new ErrorStatisticsVo();
			errorStatVo.setStat_type("M");
			
			dao.insertErrorStatistics(errorStatVo);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			CommonConstants.errorStatisticsMonthIsRunning = false;
		}
	}
